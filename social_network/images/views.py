...
def image_detail(request, id, slug):
    image = get_object_or_404(ImagesModel, slug=slug, id=id)
    return render(
        request,
        'images/image/details.html',
        {'section': 'images',
         'image': image
         }
    )
...
def image_like(request):
    ...
    if image_id and action:
        try:
            image = ImagesModel.objects.get(id=image_id)
            if action == 'like':
...
def image_list(request):
    ...
    if images_only:
        return render(request, 'images/image/images_list.html', context)
